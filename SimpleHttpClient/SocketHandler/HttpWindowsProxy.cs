...
using SafeWinHttpHandle = SimpleHttpClient.Common.Interop.WinHttp.SafeWinHttpHandle;
    ...
    internal sealed class HttpWindowsProxy : IWebProxy, IDisposable
    {
        ...
        private static void ParseProxyConfig(string value, out Uri insecureProxy, out Uri secureProxy)
        {
            ...
            if (insecureProxy == null)
            {
                ...
                if (idx >= 0)
                {
                    ...
                    int proxyLength = GetProxySubstringLength(value, idx);
                    Uri.TryCreate(string.Concat("http://", value.AsSpan(idx, proxyLength).ToArray()), UriKind.Absolute, out insecureProxy);
                }
            }
            ...
            if (idx >= 0)
            {
                ...
                int proxyLength = GetProxySubstringLength(value, idx);
                Uri.TryCreate(string.Concat("http://", value.AsSpan(idx, proxyLength).ToArray()), UriKind.Absolute, out secureProxy);
            }
        ...
        public Uri GetProxy(Uri uri)
        {
            ...
            if (_proxyHelper.ManualSettingsUsed)
            {
                ...
                return (uri.Scheme == "https" || uri.Scheme == "wss") ? _secureProxyUri : _insecureProxyUri;
            }
